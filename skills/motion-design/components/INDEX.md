...
|---|---|---|---|
| `glitterwrap` | animation | included | Send a glittering starfield of particles warping outward or inward from center, with random sparkle flashes an |
...
| `risinglines` | animation | included | Emit two layers of glowing particles — thin line sparks and soft blobs — rising or falling from a luminous hor |
...
| `pixelcard` | background-animation | included | Fill a card with a shimmering canvas grid of pixels that grow in from the center or any edge on hover, scroll, |
| `snowfall` | background-animation | included | Fill any frame with drifting canvas snow, tuning per-flake size, speed, opacity, wind sway, and fall direction |
| `emojiburst` | button | included | Launch a burst of custom emojis from a tappable button, flying off freely under gravity with a springy shake o |
...
| `boxcarousel` | image-gallery | included | Rotate images and videos across the four faces of a 3D cube, driven by autoplay, drag, or arrow keys. |
| `coverflowcarousel` | image-gallery | included | Glide through an infinite cover-flow reel where the centered image grows into a landscape hero while its neigh |
| `coverflowgallery` | image-gallery | included | A smooth 3D coverflow carousel with perspective animations, autoplay, and fully customizable titles. |
| `draggable-grid` | image-gallery | included | An infinitely draggable image canvas with smooth momentum, responsive layouts, and seamless gallery exploratio |
...
| `spinimage` | image-gallery | included | Orbit a set of images along a tilted 3D ellipse with continuous spin, depth-based scaling, and front-to-back l |
...
| `swipe-stack` | image-gallery | included | Flick through a fanned 3D stack of image cards that tilt, scale, and cycle to the back on every swipe. |
...
| `kineticgrid` | interactive-elements | included | An interactive grid background that dynamically reacts to cursor movement with smooth attraction, animated mes |
| `particlesphere` | interactive-elements | included | Spin an interactive 3D sphere of thousands of glowing particles that scatter away from your cursor and clicks. |
...
| `pixeldrift` | interactive-elements | included | Interactive particle typography that assembles, disperses, and reacts to cursor movement with smooth physics-b |
...
| `usercursor` | interactive-elements | included | Replace the native pointer inside any surface with a spring-tracked arrow and a trailing name pill that rocks  |
...
| `inkbleed` | text | included | Create interactive ink bleed effects that react fluidly to cursor movement. |
...
| `random-letter-swap` | text | included | Swap each letter of a heading vertically on hover, revealing a duplicate glyph in a randomized, staggered orde |
| `scrambletext` | text | included | Animate text with cinematic glitch reveals and interactive character-based hover effects. |
...
| `spotlighttext` | text | included | Dim a block of text and sweep a soft cursor-following spotlight across it to reveal the bright letters underne |
...
| `typewriter` | text | included | Types out a rotating list of phrases one character at a time with a blinking cursor and a static, separately c |
