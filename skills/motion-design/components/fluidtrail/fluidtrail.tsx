...
export default function FluidSim(props: any) {
    ...
    useEffect(() => {
        ...
        function splatAt(
            ...
            gl!.uniform1f(
                gl!.getUniformLocation(pSplat, "u_aspect")!,
                aspect
            )
            gl!.uniform2f(
                gl!.getUniformLocation(pSplat, "u_point")!,
                x,
                y
            )
            gl!.uniform3f(
            ...
            gl!.uniform1f(
                gl!.getUniformLocation(pSplat, "u_radius")!,
                radius
            )
            gl!.uniform1f(
                gl!.getUniformLocation(pSplat, "u_radial")!,
                1.0
            )
            blit(velocity.write)
            velocity.swap()
        ...
        function step(dt: number) {
            ...
            gl!.uniform1f(
                gl!.getUniformLocation(pClear, "u_value")!,
                0.8
            )
            blit(pressure.write)
            pressure.swap()
            ...
            const divLoc = gl!.getUniformLocation(
                pPressure,
                "u_divergence"
            )!
            const presLoc = gl!.getUniformLocation(
                pPressure,
                "u_pressure"
            )!
